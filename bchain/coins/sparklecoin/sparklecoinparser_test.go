...
func Test_GetAddrDescFromAddress(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:    "P2PKH",
			args:    args{address: "PRHBqpv2Vqhd7U2RYQXThwkkowmbeRJDWW"},
			want:    "76a914b713397f94b512330d2be552d3ce9ba8d5dd47fb88ac",
			wantErr: false,
		},
		{
			name:    "P2SH",
			args:    args{address: "pK8TcPdSyCyeDrnC7PjDk2f9286sRtWiE4"},
			want:    "a91495024685718ebe7427a472395770bb63f1d1cbcd87",
			wantErr: false,
		},
	}
	parser := NewSparklecoinParser(GetChainParams("main"), &btc.Configuration{})
...
func Test_GetAddressesFromAddrDesc(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:    "P2PKH",
			args:    args{script: "76a914b713397f94b512330d2be552d3ce9ba8d5dd47fb88ac"},
			want:    []string{"PRHBqpv2Vqhd7U2RYQXThwkkowmbeRJDWW"},
			want2:   true,
			wantErr: false,
		},
		{
			name:    "P2PK compressed",
			args:    args{script: "2103b58c56901cdc520401deb89da699948028fff44358b2475ffc05ab3279938caeac"},
			want:    []string{"PRHBqpv2Vqhd7U2RYQXThwkkowmbeRJDWW"},
			want2:   false,
			wantErr: false,
		},
		{
			...
			args:    args{script: "41044af93e3d7e0e7abf84aff2a6d52947b27868a19241621b951b0911c3cc8402234eae7a3a34bf670ef14926bbadacf86a91e954a79242a89310d46658aa6f9659ac"},
			want:    []string{"P94PFMMKHmLRxJ6t6wmSG8Tvwm9jM2JPf4"},
			want2:   false,
			wantErr: false,
		},
		{
			name:    "P2SH",
			args:    args{script: "a91495024685718ebe7427a472395770bb63f1d1cbcd87"},
			want:    []string{"pK8TcPdSyCyeDrnC7PjDk2f9286sRtWiE4"},
			want2:   true,
			wantErr: false,
		},
		{
